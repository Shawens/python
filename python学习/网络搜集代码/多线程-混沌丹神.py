...
def get_url():
    ...
    global zj_name
    global book_name
    url2 = 'https://www.aixdzs.com/novel/混沌丹神'  # 章节
    yum_url = 'https://www.aixdzs.com'
    res = requests.get(url2)
    res.encoding = 'utf-8'
    selenium_book = re.findall(r'(<li class="chapter"><a href="/read/173/173423/.*?.html" title=".*?">.*?</a></li>)', res.text)
...
def wtie_txt():
    for t in book_text:
        book_text_v = re.findall(r'<p>(.*?)</p><p>。</p>', t)[0].replace('</p><p>', '\n')
