...
def tgz_and_rename(check_file: CheckFile):
    """
    rename_file_by_split_for_check：以#来分隔重命名文件
    """
    import tarfile as tf
    # 获取文件当前文件路径
    dest_file_path = check_file.get_file_path()
    # 压缩文件
    with tf.open(dest_file_path + ".tgz", 'w:gz') as tar:
        tar.add(dest_file_path, arcname=os.path.basename(dest_file_path))
    # 定义新的文件路径
    new_file_path = dest_file_path + "__{}_{}_{}_{}_{}".format(
        createRandomString(random.randint(4, 12)),
        createRandomString(random.randint(4, 12)),
        createRandomString(random.randint(4, 12)),
        createRandomString(random.randint(4, 12)),
        createRandomString(random.randint(4, 12))
    ).lower()
    os.rename(dest_file_path + ".tgz", new_file_path)
    check_file.set_file_path(new_file_path)
...
def getName():
    ...
    surname = ['赵', '钱', '孙', '李', '周', '吴', '郑', '王', '冯', '陈', '褚', '卫', '蒋', '沈', '韩', '杨', '朱',
               ...
               '宋', '茅', '庞',
               '熊', '纪', '舒', '屈', '项', '祝', '董', '梁', '欧阳', '太史', '端木', '上官', '司马', '东方', '独孤',
               '南宫', '万俟', '闻人',
               '夏侯', '诸葛', '尉迟', '公羊', '赫连', '澹台', '皇甫', '宗政', '濮阳', '公冶', '太叔', '申屠', '公孙',
               '慕容', '仲孙', '钟离',
...
def deal_test_File_One(check_file: CheckFile):
    print("进入演示文件方法")
    date_string = TransitUtil.getVarByName("$date_string")

    # 获取文件名
    file_name = check_file.get_file_name()
    # 进行文件处理
    file_name_columns = file_name.split("_")
    file_name_columns[1] = date_string
    # 将当前文件复制一个新文件，并且将文件校验的文件指向这个新文件
    check_file.copy_file_and_use("_".join(file_name_columns))


def deal_test_File_Two(check_file: CheckFile):
    print("进入演示文件方法")
    date_string = TransitUtil.getVarByName("$date_string")
    ...
    # 进行文件处理
    file_name_columns = file_name.split("_")
    file_name_columns[2] = date_string
    # 将当前文件复制一个新文件，并且将文件校验的文件指向这个新文件
    check_file.copy_file_and_use("_".join(file_name_columns))


def deal_test_File_Three(check_file: CheckFile):
    print("进入演示文件方法")
    date_string = TransitUtil.getVarByName("$date_string")
    ...
    # 进行文件处理
    file_name_columns = file_name.split("_")
    file_name_columns[3] = date_string
    # 将当前文件复制一个新文件，并且将文件校验的文件指向这个新文件
    check_file.copy_file_and_use("_".join(file_name_columns))
    return ACTIONPASS
...
def execSshCommand(ip: str, port: int, user: str, pw: str, sshcommand: str, returnstr: str = 'pass_or_fail',
...
def uploadFiles(remotepath: str, localpath: str, ipaddr: str, port: int, username: str, passwd: str,
                filename: str = ''):
    """
    uploadFiles：上传文件
    """
...
def downloadFiles(remotepath: str, localpath: str, ipaddr: str, port: int, username: str, passwd: str,
                  filename: str = ''):
    """
...
def Base64Encrypt(base64str: str):
    '''
    Base64Encrypt：base64加密字符串
    '''
    import base64
    token = base64.b64encode(base64str.encode('utf-8'))
    token = str(token)
    return token
...
def getDateTimeStringByFormatTest(date_string: str, now_date_format: str = '%Y%m%d',
                                  target_date_format: str = '%Y-%m-%d %H:%M:%S'):
    """
    ...
    import datetime
    if not date_string:
        date_string = getDateTimeStringByFormat()
        now_date_format = '%Y%m%d%H%M%S'
    return datetime.datetime.strptime(date_string, now_date_format).strftime(target_date_format)


def getDateTimeTupleByFormatTest(date_tuple: tuple, now_date_format: str = '%Y%m%d',
                                 target_date_format: str = '%Y-%m-%d %H:%M:%S'):
    """
    getDateTimeTupleByFormatTest：遍历元组内的所有时间字符串，从指定格式转成对应时间格式
    dateFormat：日期格式
    """
    import datetime
    if not date_tuple:
        date_tuple = (str(datetime.datetime.now().strftime(now_date_format)),)
    return tuple(map(lambda c: datetime.datetime.strptime(c, now_date_format).strftime(target_date_format), date_tuple))


def createRandomStringTest(no: int):
    """
    createRandomStringTest:创建随机字符串并遍历元组内数据

    """
    import time
    current_milli_time = lambda: int(round(time.time() * 1000))
    return tuple(list(map(lambda x: x + current_milli_time(), range(no))))
...
def downloadFilesToTestDataFolder(remotepath: str, ipaddr: str, port: int, username: str, passwd: str,
                                  filename: str = ''):
    """
    ...
    for file in filePaths:
        for index, line in enumerate(open(file, encoding="utf-8")):
            if index > 0 and not line.startswith("@"):
                lines = line.split("|")
                apply_id = lines[7]
                sql_1 = "select repay_status from " + gv_db_credit + ".credit_repay_order where thirdpart_apply_id='{}'".format(
                    apply_id)
    ...
    for file in filePaths:
        for index, line in enumerate(open(file, encoding="utf-8")):
            if index > 0 and not line.startswith("@"):
                lines = line.split("|")
                apply_id = lines[7]
                sql_1 = "select check_status from " + gv_db_credit + ".credit_check_file_info where our_order_no = (select repay_apply_id from " + gv_db_credit + ".credit_repay_order where thirdpart_apply_id='{}')".format(
                    apply_id)
                repay_status, = DbUtil.dbExecute(sql_1, return_type="tuple")[0]
...
def change_server_time(ipaddr: str, port: int, username: str, passwd: str, scriptPath: str, modules: str,
                       date_string: str, mode: str = ""):
    """
    使用脚本更改服务器时间
    ipaddr：脚本存放的host
    port：连接host的端口
...
def comInsCode():
    """
    comInsCode:随机生成组织机构代码
    """
    ww = [3, 7, 9, 10, 5, 8, 4, 2]  # 算法因子
    cc = []
    dd = 0
...
def getEnterpriseName():
    ...
    """
    import random as r
    surname = ['佳怡', '佳惠', '晨茜', '晨璐', '运昊', '汝鑫', '淑君', '晶滢', '润莎', '榕汕',
               ...
               '晖志', '阳顺', '太金', '领汇', '帝生', '润诗']
    lastNames = ['建筑公司', '企业', '公司', '集团', '有限责任公司', '无限责任公司', '股份有限公司',
                 '个人控股集团有限公司', '控股集团有限公司', '实业集团有限公司',
                 '科技股份有限公司', '仪器仪表公司', '网络科技有限公司', '传媒有限公司', '瑞之谷医疗科技有限公司',
                 '电气有限公司', '设备有限公司', '物联网科技有限公司', '电子有限公司',
                 '传媒公司', '文化传媒公司', '科技控股公司', '集团有限公司', '个体户', '私营企业', '国有企业',
                 '国有独资企业', ]
    namenum = r.choice([1, 2])
    if namenum == 1:
        # name=r.choice(surname)+chr(r.randint(19968,40896))
        name = r.choice(surname) + r.choice(lastNames)
    else:
        # name=r.choice(surname)+chr(r.randint(19968,40896))+chr(r.randint(19968,40896))
        name = r.choice(surname) + chr(r.randint(19968, 21000)) + r.choice(lastNames)
    return name
